        ...
        <div className="flex gap-1 rounded-lg bg-gray-800 p-1">
          {Object.entries(billing).map(([key, info]) => (
            ...
            >
              {info.label}
              {info.discount > 0 && (
                <span className="ml-1 text-green-400">-{(info.discount * 100).toFixed(0)}%</span>
              )}
            </button>
          ))}
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {plans.map((plan) => {
          ...
          return (
            <div
              key={plan.id}
              className={`relative rounded-xl border bg-gray-900 p-6 transition-shadow hover:shadow-lg ${
                isCurrent ? `${planBorders[plan.id]} shadow-lg` : 'border-gray-800'
              }`}
            >
              {isCurrent && (
                ...
                {Object.entries(plan.features).map(([key, enabled]) => (
                  <li key={key} className="flex items-center gap-2 text-sm">
                    {enabled ? (
                      ...
                      <X size={16} className="shrink-0 text-gray-600" />
                    )}
